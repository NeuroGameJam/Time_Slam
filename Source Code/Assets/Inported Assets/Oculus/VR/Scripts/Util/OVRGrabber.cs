...
public class OVRGrabber : MonoBehaviour
{
	...
	public Dictionary<OVRGrabbable, int> m_grabCandidates = new Dictionary<OVRGrabbable, int>();
	protected bool operatingWithoutOVRCameraRig = true;
    public List<OVRGrabbable> farGrabbables = new List<OVRGrabbable>();


    public OVRGrabbable lookingGrabbable;
    public Button3D lookingButton;
    public bool isClickingTheButton { get; private set; }
    ...
    protected virtual void GrabBegin()
    {
        ...
        if (closestGrabbable != null)
        {
            ...
            if (PickFarObj.instance)
            {
                if (m_grabbedObj == lookingGrabbable)
                {
                    Debug.Log("grab a pointing");
                    m_grabbedObj.transform.position = m_gripTransform.position;
                }
            }
    ...
    public void IsLookingAtAGrabbable()
    {
        foreach (OVRGrabbable grabbable in m_grabCandidates.Keys)
        {
            float angle = FindAngleFromObj(grabbable.transform);
            //Debug.Log(angle);
            if (angle <= 45)
            {
                grabbable.GetComponent<MeshRenderer>().material.color = Color.red;
                farGrabbables.Add(grabbable);
            ...
            {
                if(farGrabbables.Contains(grabbable))
                {
                    farGrabbables.Remove(grabbable);
                ...
                grabbable.GetComponent<MeshRenderer>().material.color = Color.white;

            }
        }
    }

    protected float FindAngleFromObj(Transform obj)
    {
